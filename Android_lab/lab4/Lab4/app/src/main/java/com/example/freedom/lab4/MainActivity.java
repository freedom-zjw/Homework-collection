...
public class MainActivity extends AppCompatActivity {
    ...
    final String STATICACTION = "StaticFilter";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //静态广播，在启动应用时会有通知产生随机推荐一个商品
        Random random = new Random();
        int rand = random.nextInt(items.size());
        Intent intentBroadcast = new Intent(STATICACTION);
        intentBroadcast.putExtra("Name", items.get(rand).getName());
        intentBroadcast.putExtra("Price", items.get(rand).getPrice());
        intentBroadcast.putExtra("Info", items.get(rand).getInfo());
    ...
    public void noMessageEvent(MessageEvent messageEvent){
        shopcaritems.add(new Items(messageEvent.getName(),messageEvent.getInfo(),messageEvent.getPrice()));
        shopcarAdapter.notifyDataSetChanged();
    }
    @Override
