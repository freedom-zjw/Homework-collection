clc
clear all;
fp=3500;wp=2*pi*fp;        %输入实际滤波器的通带截止频率
fs=6000;ws=2*pi*fs;        %输入实际滤波器的阻带截止频率
...
%变换为实际模拟带通滤波器
[b1,a1]=lp2lp(b0,a0,wc);    %从归一化低通变换到实际低通
[Ha,wa]=freqs(b1,a1);       %求实际低通滤波器的频率特性
