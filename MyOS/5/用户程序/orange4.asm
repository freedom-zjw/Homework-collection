...
START:
	...
	cmp byte[run],1
	jz BEGIN
	mov word[x1],25 ;字符坐标初始化为（-1，-1），这样字符将从最左上角运动
	mov word[y1],80
	mov byte[state],topL	;运动方向初始化为往左上方向
...
DnR:
	...
	cmp word[x1],24 ;判断是否到达下边界，到达则跳转到朝右上方向运动
	jz UpR
	cmp word[y1],79 ;判断是否到达右边界，到达则跳转到朝左下方向运动
	jz DnL
...
ShowID:
	mov word[x2],17 ;选择中间的位置开始显示字符串
	mov word[y2],58
	mov si,0	;用于记录字符串显示到第几位
	mov cx,4	;循环次数
L1:	
	call Calp	;调用Calp计算偏移地址
	mov al,byte[line1+si] ;显示字符串的第si位
	mov byte[es:bx],al
