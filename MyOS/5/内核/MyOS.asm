...
start:
	xor ax,ax
	mov es,ax
	mov word ptr es:[51*4],offset interrupt33h
	mov ax,cs
	mov word ptr es:[51*4+2],ax
	
	xor ax,ax
	mov es,ax
	mov word ptr es:[52*4],offset interrupt34h
	mov ax,cs
	mov word ptr es:[52*4+2],ax
	
	xor ax,ax
	mov es,ax
	mov word ptr es:[53*4],offset interrupt35h
	mov ax,cs
	mov word ptr es:[53*4+2],ax
	
	xor ax,ax
	mov es,ax
	mov word ptr es:[54*4],offset interrupt36h
	mov ax,cs
