...
void GetInput()
{
	...
	while( input != '\n'&& input != '\r' ){
		if(input=='\b'){
			...
			getChar();
			if(input=='\b' && index)Print("\b");
			else if (input!='\b')printChar(input);
		}
		else{
			Buffer[index++] = input;
			getChar();
...
{
	Print("                        \n    Welcome to MyOS!\r\n");
	Print("     Here you can excute some functions for fun by key in specific commands.\r\n");
	Print("  Please input \"help\" to see what commands that you can key in !\r\n");
}
