...
if __name__ == "__main__":
    """
    writer = csv.writer(open("F1.csv", "w", encoding="utf-8", newline=""))
    for i in range(1, 51):
        traindataSet, trainLabels, valdataSet, valLabels = loadDataSet('newTrain2.csv')
        StrongClassifier = train(traindataSet, trainLabels, i)
        ...
        print("")
    """
    reader = csv.reader(open("F1.csv","r", encoding='utf-8'))
    F1 = []
    for row in reader:
        F1.append([float(row[0]), float(row[1])])
    F1 = np.array(F1)
    trainF1 = F1[:, 0]
    valF1 = F1[:, 1]
    x = [i+1 for i in range(50)]
    plt.plot(x, trainF1, label="trainF1")
    plt.plot(x, valF1, label='valF1')
    plt.legend()
    plt.show()
