function []=regression()
    ...
    %%------验证集划分---------------三个验证集随意
    G_train=train(5726:14491,:);
    G_train_ans=train_ans(5726:14491,:);
    ...
    G_val3=train(15920:16637,:);
    G_val_ans3=train_ans(15920:16637,:);
    ...
    [MSE,corr,MSE_val,corr_val,out1,out2,best_w,best_b]= BPNN(G_train, G_val,ow,ob,G_iter,G_Node, layers,G_step,G_lambda,G_alpha,G_train_ans,G_val_ans,single(size(G_train_ans,1)),single(size(G_val_ans,1)));
    %--------------------------以下为作图调参区 现在不用了
    % figure;
...
%     best_b=theta;
    ...
    MSE=zeros(1, iter,'single');
    MSE_val=zeros(1, iter,'single');
    corr=zeros(1,iter,'single');
    corr_val=zeros(1,iter,'single');
